...
void	perform_vertical_ray_cast(t_all *s)
{
	...
	x = (int)(s->pos.x) + s->ray.v;
	y = s->pos.y + ((x - s->pos.x) * (s->ray.y / s->ray.x));
	while ((int)(y) > 0 && (int)(y) < s->map.y)
	{
		if (s->map.tab[(int)(y)][(int)(x - 1 + s->ray.v)] == '1')
		{
			s->hit.x = x;
			s->hit.y = y;
...
void	perform_horizontal_ray_cast(t_all *s)
{
	...
	y = (int)(s->pos.y) + s->ray.w;
	x = s->pos.x + (y - s->pos.y) * (s->ray.x / s->ray.y);
	map_width = 0;
	while (1)
	{
		map_width = ft_strlen(s->map.tab[(int)(y)]);
		if ((int)(x) <= 0 || (int)(x) >= map_width)
			break ;
		if (s->map.tab[(int)(y - 1 + s->ray.w)][(int)(x)] == '1')
		{
